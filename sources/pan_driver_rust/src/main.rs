...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    loop {
        ...
        let midi_out = match MidiOutput::new("The Pan Output") {
            Ok(out) => out,
            Err(msg) => {
                eprintln!("Error creating MIDI output. {msg}");
                continue;
            }
        };
        let midi_port: MidiOutputPort =
            match find_virtual_midi_port_by_name(&virtual_midi_port_name, &midi_out) {
                Some(port) => port,
                None => {
                    eprintln!("Couldn't find virtual midi port {virtual_midi_port_name}");
                    continue;
                }
            };
        ...
        loop {
            match loopmidi_process {
                ...
                LoopMidiProcess::External(pid) => {
                    // Check if loopMIDI is still running
...
impl Settings {
    fn load_from_file() -> Result<Self, ConfigError> {
        ...
        let baud_rate = settings.get_int("port.serial_port_baudrate")? as u32;

        let loopmidi_path = settings.get_string("loopmidi.path")?;

        let virtual_midi_port_name = settings.get_string("port.virtual_midi_port_name")?;
